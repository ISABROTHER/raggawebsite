...
import {
  ...
  Minus, Plus, Loader2, Smartphone, CreditCard, Landmark, Apple
} from 'lucide-react';
...
export function DonationModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const startAdjusting = (direction: 'up' | 'down') => {
    stopAdjusting();
    adjust(direction); // Single tap action
    timeoutRef.current = setTimeout(() => {
      intervalRef.current = setInterval(() => adjust(direction), 60);
    }, 400); // Auto-scroll after 400ms
  };
      ...
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl relative animate-in zoom-in-95 duration-300 max-h-[95vh] flex flex-col border border-white/20">
        ...
        <div className="bg-green-700 p-5 md:p-6 text-white flex justify-between items-center shrink-0">
          <div className="text-left">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-100 mt-1">Building With You • Obiara Ka Ho</p>
          </div>
          {!isProcessing && (
        ...
        <div className="p-6 md:p-8 overflow-y-auto overscroll-contain">
          {isProcessing ? (
            <div className="py-12 flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-slate-400">Directing to Paystack...</p>
            </div>
          ) : (
            <>
              {step === 1 && (
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  <div className="flex items-center gap-2">
                    <span className="flex items-center justify-center w-6 h-6 rounded-full bg-green-700 text-white font-bold text-[10px]">1</span>
                    <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-400">Select Impact</h3>
                  </div>

                  <div className="grid grid-cols-4 gap-2">
                    {[500, 1000, 5000, 10000].map((num) => (
                      <button key={num} onClick={() => setSelectedAmount(num)} className={`py-3.5 rounded-2xl border-2 font-bold text-xs transition-all ${selectedAmount === num ? 'border-green-700 bg-green-50 text-green-700' : 'border-slate-100 bg-white text-slate-400'}`}>
                        {num.toLocaleString()}
                      </button>
                    ))}
                  </div>
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-1.5 overflow-hidden">
                        <input 
                          ...
                          style={{ fontSize: '32px' }}
                        />
                        <span className="text-3xl font-bold text-slate-900 tracking-tighter">BOOKS</span>
                      </div>

                      <div className="flex items-center gap-2">
                        <button onMouseDown={() => startAdjusting('down')} onTouchStart={() => startAdjusting('down')} onMouseUp={stopAdjusting} onTouchEnd={stopAdjusting} className="w-11 h-11 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-900 shadow-sm active:scale-90"><Minus className="w-5 h-5" strokeWidth={3} /></button>
                        <button onMouseDown={() => startAdjusting('up')} onTouchStart={() => startAdjusting('up')} onMouseUp={stopAdjusting} onTouchEnd={stopAdjusting} className="w-11 h-11 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-900 shadow-sm active:scale-90"><Plus className="w-5 h-5" strokeWidth={3} /></button>
                      </div>
                    </div>
                  ...
                  <div className="bg-green-700 p-6 rounded-[2rem] text-center text-white shadow-xl shadow-green-100 relative overflow-hidden">
                    ...
                    <p className="relative z-10 text-[9px] font-bold tracking-widest text-white/30 uppercase mt-1">≈ ${totalUSD.toLocaleString(undefined, { maximumFractionDigits: 2 })} USD</p>
                  </div>

                  {/* NDC RED ANIMATED BUTTON */}
                  <button 
                    onClick={() => setStep(2)} 
                    className="group relative w-full py-4.5 bg-red-600 text-white rounded-2xl font-bold uppercase tracking-[0.2em] text-[11px] shadow-xl transition-all duration-300 hover:bg-red-700 active:scale-95 flex items-center justify-center gap-2 overflow-hidden"
              ...
              {step === 2 && (
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  <div className="flex items-center gap-2">
                    <span className="flex items-center justify-center w-6 h-6 rounded-full bg-green-700 text-white font-bold text-[10px]">2</span>
                    <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-400">Sponsor Details</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      ...
                      { id: 'BANK', label: 'Bank', icon: Landmark, color: 'text-red-500' },
                      { id: 'APPLE', label: 'Apple', icon: Apple, color: 'text-slate-900' }
                    ].map((m) => (
                      <button key={m.id} onClick={() => setPayMethod(m.id as any)} className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${payMethod === m.id ? 'border-green-700 bg-green-50' : 'border-slate-100 bg-white opacity-60'}`}>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <input type="text" placeholder="First Name" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full h-14 px-5 bg-slate-50 border-none rounded-2xl font-bold text-base outline-none focus:ring-2 focus:ring-green-700/10" style={{ fontSize: '16px' }} />
                      <input type="text" placeholder="Surname" value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full h-14 px-5 bg-slate-50 border-none rounded-2xl font-bold text-base outline-none focus:ring-2 focus:ring-green-700/10" style={{ fontSize: '16px' }} />
                    </div>
                    <input type="text" placeholder={payMethod === 'MOMO' ? "Phone Number" : "Email Address"} value={contactInfo} onChange={(e) => setContactInfo(e.target.value)} className="w-full h-14 px-5 bg-slate-50 border-none rounded-2xl font-bold text-base outline-none focus:ring-2 focus:ring-green-700/10" style={{ fontSize: '16px' }} />
                  </div>
                  ...
                  <div className="pt-2 flex gap-3">
                    ...
                    <button 
                      ...
                      className="group relative w-2/3 py-5 bg-red-600 text-white rounded-2xl font-bold uppercase text-xs tracking-[0.2em] shadow-xl transition-all duration-300 active:scale-95 overflow-hidden disabled:opacity-30"
                      style={{ animation: (!firstName || !lastName || !contactInfo) ? 'none' : 'breathing-red 2.5s infinite ease-in-out' }}
                    >
                      <div className="absolute inset-0 w-1/3 h-full bg-white/10 skew-x-[-25deg] -translate-x-[250%] group-hover:animate-[shimmer-btn_1.5s_infinite]" />
                      <span className="relative z-10 uppercase">Confirm Donation</span>
                    </button>
                  </div>
                </div>
              )}
              ...
              {step === 3 && (
                <div className="text-center py-10 animate-in zoom-in-95 duration-500">
                  ...
                  <p className="text-sm text-slate-400 font-medium leading-relaxed max-w-[240px] mx-auto mb-10">Generational impact recorded. Obiara Ka Ho.</p>
                  <button onClick={handleClose} className="w-full py-5 border border-slate-100 text-slate-900 rounded-2xl font-bold uppercase tracking-widest text-[10px] active:scale-95">Close Portal</button>
                </div>
              )}
            </>
          )}
