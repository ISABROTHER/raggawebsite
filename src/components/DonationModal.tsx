...
import { 
  ...
  User, Phone, ShieldCheck, CreditCard, Apple, Loader2, BarChart3, TrendingUp 
} from 'lucide-react';
          ...
          {isProcessing ? (
            <div className="py-12 flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-[10px] font-bold text-slate-400 text-center uppercase tracking-wide px-6">
                {currency === 'GHS' ? 'Check your phone to authorize.' : 'Please do not refresh the page.'}
              </p>
            </div>
          ) : (
            <>
              ...
              {step === 1 && (
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="bg-green-700 p-5 rounded-2xl text-center text-white shadow-lg">
                    <p className="text-3xl font-black">{currency === 'GHS' ? '₵' : '$'}{totalCost.toLocaleString()}</p>
                  </div>
                  <button onClick={() => setStep(2)} className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-sm">Next Step <ChevronRight className="w-4 h-4 ml-2 inline"/></button>
                </div>
              )}

              {/* --- STEP 2: DYNAMIC PAYMENT METHOD --- */}
              {step === 2 && (
                <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      <input type="text" placeholder={currency === 'GHS' ? "MoMo Number" : "Card Info / Email"} className="w-full pl-11 pr-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-sm" />
                    </div>
                  </div>
                  <div className="pt-4 flex gap-3">
                    ...
                    <button onClick={handlePay} className="w-2/3 py-5 bg-red-800 text-white rounded-2xl font-black uppercase text-sm shadow-xl">Proceed to Pay</button>
                  </div>
                </div>
              )}
              ...
              {step === 3 && (
                <div className="text-center py-6 animate-in zoom-in-95 duration-500">
                  ...
                  <p className="text-sm text-slate-600 font-medium leading-relaxed max-w-xs mx-auto mb-8">Your contribution of <span className="font-black text-green-700">{currency === 'GHS' ? '₵' : '$'}{totalCost}</span> for {selectedAmount} books has been recorded.</p>
